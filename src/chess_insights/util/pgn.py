...
def convert_move_pgn(origin_square: int,
    ...
    if piece.piece_type == ChessPieceType.KING and chebyshev_distance(origin_square,
        ...
        move_notation = f"{piece.fen if piece.fen != 'P' else ''}{'x' if is_capture else ''}{Square(target_square).name}"
    ...
    pgn += "+" if is_check else ""
    pgn += "#" if status == GameStatus.CHECKMATE else ""
