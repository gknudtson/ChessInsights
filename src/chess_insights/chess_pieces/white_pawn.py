...
class WhitePawn(ChessPiece):
    ...
    def move(self, origin_square: int, target_square: int):
        ...
        if not self.chess_board.is_whites_turn():
            return
        if not self.chess_board.is_piece_on_square("white", "pawn", origin_square):
            return
        ...
        if self.__is_valid_move:
            ...
            self.chess_board.remove_piece("white", "pawn", origin_square)
            self.chess_board.change_turn()

    def capture(self, target_square: int):
        if self.chess_board.is_piece_on_square("black", "piece", target_square):
    ...
    def en_passant(self, target_square: int):
        self.chess_board.remove_piece("black", "pawn", target_square - 8)
        self.__is_valid_move = True
